...
def upload(request):
    if request.method == 'POST':
            ...
            for line in file:
                try:
                    ...
                    return HttpResponseBadRequest(
                        'Not a text file or not utf-8 encoded. try using the text field below.'
                    )
            lines = lines.split('\n')
        ...
        if request.POST.get("left-margin") and int(
                request.POST.get("left-margin")) > 0:
            left_margin = int(request.POST.get("left-margin"))
        if request.POST.get("right-margin") and int(
                request.POST.get("right-margin")) > 0:
            right_margin = int(request.POST.get("right-margin"))
        if request.POST.get("top-margin") and int(
                request.POST.get("top-margin")) > 0:
            top_margin = int(request.POST.get("top-margin"))

        return process(lines, num_images, search_term, color, fontsize,
                       fonttype, left_margin, top_margin, right_margin)
    else:
...
def process(lines, num_images, search_term, color, fontsize, fonttype,
    ...
    count = 1
    r = requests.get(
        f'https://api.unsplash.com/photos/random?query={search_term}&count={num_images}',
    ...
    for entry in r.json():
        ...
        offset = top_margin
        line = lines[lineidx]
        for line in wrapword(line, img.width - (left_margin + right_margin),
                            myFont):
            I1.text((left_margin-1, offset), line, "#333", font=myFont)
            I1.text((left_margin+1, offset), line, "#333", font=myFont)
            I1.text((left_margin, offset-1), line, "#333", font=myFont)
            I1.text((left_margin, offset+1), line, "#333", font=myFont)
        ...
        I1.text((9, 10), f"image by: {name}", "#333", font=myFont)
        I1.text((11, 10), f"image by: {name}", "#333", font=myFont)
        I1.text((10, 9), f"image by: {name}", "#333", font=myFont)
        I1.text((10, 11), f"image by: {name}", "#333", font=myFont)
